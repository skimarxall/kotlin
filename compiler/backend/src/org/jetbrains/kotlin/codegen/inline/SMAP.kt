...
object SMAPBuilder {
    fun build(fileMappings: List<FileMapping>, backwardsCompatibleSyntax: Boolean): String? {
        val realMappings = fileMappings.filter { it.lineMappings.isNotEmpty() && it.lineMappings.first() != RangeMapping.SKIP }
        if (realMappings.isEmpty()) {
            return null
        }
        val source = fileMappings[0].name
        val path = fileMappings[0].path
        val defaultStrata = generateDefaultStrata(realMappings)
        val debugStrata = generateDebugStrata(realMappings, source, path)
    ...
    private fun generateDebugStrata(realMappings: List<FileMapping>, source: String, path: String): String {
...
class NestedSourceMapper(
    ...
    override fun mapLineNumber(lineNumber: Int): Int {
        ...
        val newLineNumber = when {
            sourceLineNumber == -1 -> -1
            sameFile -> parent.mapLineNumber(sourceLineNumber, range.parent!!.name, range.parent.path, range.callSiteMarker)
            else -> parent.mapLineNumber(sourceLineNumber, range.parent!!.name, range.parent.path)
        }
        if (newLineNumber > 0) {
...
class FileMapping(val name: String, val path: String) {
    ...
    companion object {
        val SKIP = FileMapping("no-source-info", "no-source-info").apply {
            lineMappings.add(RangeMapping.SKIP)
        }
    }
}

data class RangeMapping(
    val source: Int, val dest: Int, var range: Int = 1, val callSiteMarker: CallSiteMarker? = null,
    val parent: FileMapping? = null
) {
